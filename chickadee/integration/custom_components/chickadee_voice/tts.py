...
"""Chickadee Voice text-to-speech entity — asks the add-on's TTS engine for spoken audio.
...
async def async_setup_entry(
    ...
    async_add_entities([ChickadeeVoiceTtsEntity()])


class ChickadeeVoiceTtsEntity(tts.TextToSpeechEntity):
    """One TTS entity per install; the add-on routes to the configured engine."""

    _attr_name = "Chickadee Voice TTS"
    ...
    async def async_added_to_hass(self) -> None:
        ...
        try:
            ...
            if status < 400:
                ...
                _LOGGER.info("Chickadee Voice TTS voice catalog: %d voices", len(self._voices))
        except (AddonUnavailable, ValueError, UnicodeDecodeError) as err:
            _LOGGER.debug("Chickadee Voice TTS voice catalog unavailable: %s", err)
    ...
    async def async_get_tts_audio(
        ...
        try:
            ...
            _LOGGER.warning("DROP: Chickadee Voice TTS — add-on unavailable: %s", err)
            return None, None

        if status >= 400 or not body or "audio" not in (ctype or ""):
            _LOGGER.warning("DROP: Chickadee Voice TTS — add-on HTTP %s (%s): %s", status, ctype, body[:200])
            return None, None
