...
"""Chickadee Voice speech-to-text entity — streams Assist audio to the add-on's STT engine.
...
async def async_setup_entry(
    ...
    async_add_entities([ChickadeeVoiceSttEntity()])


class ChickadeeVoiceSttEntity(stt.SpeechToTextEntity):
    """One STT entity per install; the add-on routes to the configured engine."""

    _attr_name = "Chickadee Voice STT"
    ...
    async def async_process_audio_stream(
        ...
        try:
            ...
            _LOGGER.warning("DROP: Chickadee Voice STT — add-on unavailable: %s", err)
            return stt.SpeechResult(None, stt.SpeechResultState.ERROR)

        if status >= 400:
            _LOGGER.warning("DROP: Chickadee Voice STT — add-on HTTP %s: %s", status, body[:200])
            return stt.SpeechResult(None, stt.SpeechResultState.ERROR)
        try:
            text = str(json.loads(body.decode("utf-8")).get("text") or "")
        except (ValueError, UnicodeDecodeError) as err:
            _LOGGER.warning("DROP: Chickadee Voice STT — unparseable add-on response: %s", err)
            return stt.SpeechResult(None, stt.SpeechResultState.ERROR)
