...
"""Chickadee Voice conversation entity — routes Assist utterances to the Chickadee brain.

Port of the Chickadee integration's conversation entity (device-verified there), with
...
async def async_setup_entry(
    ...
    async_add_entities([ChickadeeVoiceConversationEntity(config_entry)])


class ChickadeeVoiceConversationEntity(conversation.ConversationEntity):
    ...
    async def async_process(
        ...
        try:
            ...
            _LOGGER.warning("DROP: Chickadee Voice conversation — add-on unavailable: %s", err)
            return self._result(
            ...
            _LOGGER.warning("DROP: Chickadee Voice conversation — brain call failed: %s", err)
        ...
        for act in actions:
            ...
            for cmd in (act.get("parameters") or {}).get("commands") or []:
                ...
                try:
                    ...
                    _LOGGER.info("Chickadee Voice executed HA %s.%s %s", domain, service, data)
                except Exception as err:  # noqa: BLE001
