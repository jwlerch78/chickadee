...
// auth.js — persistent JWT storage + Chickadee device-flow auth.
// Ported from the Chickadee add-on's auth.js (same account system, same jwt-auth
...
async function createDeviceCode() {
    ...
    // auth page, preserving the query (code + type). Same fix as the Chickadee add-on.
    ...
    // Skin for the approval page (brand-config.js in the Chickadee webapp).
    //
    ...
    // is live, this falls back to the full-scope Chickadee brand.
...
async function refreshJwt(currentJwt) {
    ...
    if (!resp.ok) {
        ...
        // Deleted account is definitive — drop the credential (Chickadee 2026-07-13 lesson:
