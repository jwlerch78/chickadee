...
//   (device mgmt)     getDeviceByChickadeeId / renameDevice / getEntitiesForHaDevice /
...
let registryCache = null;           // { byChickadeeId: Map, byHaId: Map, fetchedAt }
...
async function _refreshRegistryCache() {
    ...
    registryCache = { byChickadeeId, byHaId, fetchedAt: Date.now() };
...
async function getDeviceByChickadeeId(dashieDeviceId, { force = false } = {}) {
    if (!dashieDeviceId) return null;
    if (force || !registryCache) await _refreshRegistryCache();
    let entry = registryCache.byChickadeeId.get(dashieDeviceId);
    if (!entry && !force) {
        // Stale cache (device was just added/migrated) — refresh once and retry
        await _refreshRegistryCache();
        entry = registryCache.byChickadeeId.get(dashieDeviceId);
    }
    ...
    // ends in `_device_id` by convention (ChickadeeDeviceIdSensor sets
...
/**
 * Set the user-facing name (`name_by_user`) for the device matching this Chickadee
 * device_id. Returns the updated registry entry, or throws on failure.
 */
async function renameDevice(dashieDeviceId, newName) {
    const entry = await getDeviceByChickadeeId(dashieDeviceId);
    if (!entry) {
        throw new Error(`No HA device_registry entry found for Chickadee device_id ${dashieDeviceId}`);
    }
 ...
 *  voice list when the account's Chickadee language is 'system' (the default): a box's voice
...
/** Raw HA state list. Used by LAN discovery to read the `ip_address` attribute Chickadee
