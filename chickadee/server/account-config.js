...
// and healthy, and making the perpetual voice license a lie the moment Chickadee had a bad day.
//
...
async function getAccountVoiceConfig() {
    ...
    if (jwt && userId) {
      ...
      if (resp.ok) {
        ...
        value = {
          ...
          // key. Consumed by ChickadeeCloudCapabilityClient on the anon-kiosk path.
          pipeline: {
  ...
  if (readOk) {
    ...
    // guessing 'cloud' — a local-first household keeps working even if Chickadee is gone for
    ...
    if (cached) {
      ...
      console.warn('[account-config] using last-known-good config from /data'
        + ` (model=${cached.model || 'none'}) — Chickadee unreachable, local setup unaffected.`);
    } else {
