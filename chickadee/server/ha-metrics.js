...
// Pure functions for turning raw HA /api/states output into per-Chickadee-device
...
//      " Device ID". Each anchor IS one Chickadee device in the HA integration.
...
//
// Why anchor on _device_id instead of friendly_name: the Chickadee HA integration does
// NOT put "Chickadee" in every device's friendly_name — only some of them do (e.g.,
// "Mio 15\" Chickadee"). But every device has a `sensor.<slug>_device_id` — that's the
 ...
 *  in " Device ID" is a Chickadee-integration device anchor. */
...
function _buildViaRegistry(states, entityRegistry) {
    ...
    for (const anchor of anchors) {
        ...
        for (const entity of deviceEntities) {
            ...
            // it to this Chickadee device, so there's no cross-device leakage
