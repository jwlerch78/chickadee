...
// Kokoro/Piper-shim (/v1/audio/speech) — the same "Local Engines" lane the Chickadee
// console productized. The hosted (Chickadee) engines slot into this same
...
async function handleStt(req, res, sendJson) {
    ...
    if (!base) {
        // Hosted fallback: Chickadee Whisper under the account (metered).
...
async function handleTts(req, res, sendJson) {
    ...
    if (!base) {
        ...
        // Hosted fallback: Chickadee voice under the account (metered).
        const jwt = await cloudJwt();
        if (!jwt) {
