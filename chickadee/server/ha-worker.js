...
let slugByChickadeeId = {};    // dashie_device_id → HA entity slug (e.g. 'fire_tv', 'rk3576_u')
...
async function runPoll(reason = 'tick') {
    ...
    try {
        ...
        // buildDeviceMetrics — so on a box with no Chickadee account the worker
        ...
        // subtle change here would fail QUIET — a Chickadee box would keep polling
        ...
        // entity_ids by Chickadee device_id. entity_ids is the authoritative
        ...
        slugByChickadeeId = newSlugMap;
        entityIdsByChickadeeId = newEntityIdMap;
        ...
        if (devices.length === 0) {
            ...
            console.log('[ha-worker] No Chickadee entities found in HA.');
            return;
        }
...
function getSlugForDevice(dashieDeviceId) {
    return slugByChickadeeId[dashieDeviceId] || null;
}
...
function getEntityIdForRole(dashieDeviceId, role) {
    return entityIdsByChickadeeId[dashieDeviceId]?.[role] || null;
}
