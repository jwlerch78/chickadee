...
// integration-installer.js — install/update the Chickadee Voice INTEGRATION from the
 ...
 *
 * 🔴 Why a PATTERN and not a `LEGACY_MARKERS = ['.installed_by_chickadee_addon']`
 * list, which is the obvious fix and is broken here:
 *
 * This file is brand-generated. A legacy literal written in canon would be
 * SUBSTITUTED on the way into the other brand's tree — `.installed_by_chickadee_addon`
...
async function ensureIntegration() {
    try {
        ...
        if (installed === null) {
            ...
            await notifyRestart(
                `The Chickadee Voice integration (v${bundled}) was installed. ` +
                '**Restart Home Assistant** (Settings → System → Restart) to activate it — '
                + 'Chickadee Voice will then appear as a discovered device to configure with one click.');
            return 'installed';
        }
        ...
        if (installedHash !== bundledHash) {
            ...
            await notifyRestart(
                `The Chickadee Voice integration was updated (bundled v${bundled}). ` +
