/* ============================================================
   ...
   inspectable core that ships in the Chickadee
...
const FeatureGate = {
     ...
     * the Chickadee add-on, whose brand.js declares
     ...
     * — the family product (and Chickadee device management). Checked FIRST in
    ...
    CLOSED_DELTA_PAGES: new Set([
        // 'devices' left this set on 2026-07-30 — the HA edition manages Chickadee
        ...
        // account-wide Chickadee settings (account required, either build).
    ]),
    ...
    LOCAL_MODE_PAGES: new Set([
        ...
        // a Chickadee account. Every /api/feeds/* route proxies to the integration's
        ...
        // Chickadee settings via DashieAuth (user_settings), so signed out there is
    ...
    // EMPTY IN THIS BUILD, deliberately. Upstream this set makes
    // account-only pages VISIBLE-but-locked so an HA user can learn the
    // features exist. Here there is no account to sign into, so a locked
    // entry would advertise a product that does not exist and lead to a
    // renderer whose stub this build removes. Absent beats re-worded.
     ...
     * That is right for **Chickadee**, which keeps accounts, credits and devices and
    ...
    FAMILY_ONLY_OPTIONS: {
        ...
        // supabase = Chickadee photo albums (family product).
     ...
     * Chickadee credits meter ARE the product there, so the family
    ...
    PUBLISHED_RULE_OVERRIDES: {
        ...
        // (FEATURE_RULES) is untouched, so this changes nothing for Chickadee users.
     ...
     * scoped to voice/AI. Such accounts hide the whole Chickadee dashboard
     ...
     */
    FEATURE_RULES: {
        // Voice / AI features (Chickadee token spend) — the hand-selected BETA
    ...
    ENTITLEMENT_GATED_PAGES: new Set([
        ...
        // core HA capability available to ALL HA users, independent of Chickadee
        // subscription/trial state — an expired or ha_only account still sees them.
    ]),

    /**
     * The Chickadee dashboard pages hidden for an ha_only (voice-only)
    ...
    isPageEnabled(page) {
        ...
        if (this.isPublishedBuild()) {
            ...
            // Chickadee plan/trial entitlement gating doesn't exist in the open
