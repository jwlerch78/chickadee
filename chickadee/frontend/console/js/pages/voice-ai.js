...
const VoiceAiPage = {
    ...
    selectPreset(id) {
        ...
        if (id !== 'ha_assist') {
            ...
            // HA Assist has no Chickadee brain → ai.model untouched.
     ...
     *  entities voice commands can control: the entities on the Chickadee DASHBOARD (plug-and-play —
    ...
    /** Household Chickadee Intelligence sharing toggle — add-on mode only. Lets un-logged-in
    ...
    async setHouseholdSharing(enabled) {
        try {
            ...
            // `list_devices` is a Chickadee-service call that cannot succeed here. The
    ...
    _renderAiDefaults() {
        ...
        // local Android voice) — just without the Chickadee-brain cards.
        ...
        // pipeline with local engines/Android voice) — no Chickadee-brain cards
     ...
     *   1. Chickadee's own language — the language Chickadee SPEAKS (AI prompt + cloud voice), so
     ...
     *  cost — its turns run on the user's key, not Chickadee credits. No-op off add-on
