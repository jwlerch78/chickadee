...
"""Auto-create the Chickadee Voice Assist pipeline on config-entry setup.
...
configuration. Idempotent: any existing pipeline that references a Chickadee Voice
...
async def async_ensure_pipeline(
    ...
    """Create the Chickadee Voice Assist pipeline if no pipeline references our entities.
    ...
    try:
        ...
        _LOGGER.warning(
            ...
            "Chickadee Voice pipeline, create one manually in Settings > Voice assistants (%s)",
            err,
        )
        return
    ...
    if conversation_id is None:
        _LOGGER.warning(
            "DROP: Chickadee Voice conversation entity not registered yet — pipeline not created"
        )
        return

    # Idempotency: any pipeline touching a Chickadee Voice entity — whatever the user
    ...
    for pipeline in async_get_pipelines(hass):
        if {pipeline.conversation_engine, pipeline.stt_engine, pipeline.tts_engine} & ours:
            _LOGGER.debug(
                "Chickadee Voice pipeline already present ('%s') — leaving it untouched",
                pipeline.name,
            )
            return
    ...
    if stt_id is None or tts_id is None:
        ...
        _LOGGER.warning(
            "DROP: Chickadee Voice STT/TTS entity missing (stt=%s tts=%s) — creating a "
            "reduced pipeline",
            stt_id,
            tts_id,
        )
    ...
    try:
        ...
        _LOGGER.warning("DROP: Chickadee Voice pipeline creation failed: %s", err)
        return
...
def _our_entity_ids(hass: HomeAssistant) -> set[str]:
    """The Chickadee Voice-owned pipeline entity ids currently in the registry."""
    ent_reg = er.async_get(hass)
    return {
...
async def _apply_wake_to_pipeline(
    ...
    """Set wake on the existing Chickadee Voice pipeline if it's currently unset.
