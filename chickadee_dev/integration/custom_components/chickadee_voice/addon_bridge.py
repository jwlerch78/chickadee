...
Ported from the Chickadee integration's addon_bridge (subset). The add-on owns engine
...
async def _read_bridge_secret(hass: HomeAssistant) -> str | None:
    ...
    if _secret_cache:
        _LOGGER.info("Chickadee Voice bridge secret loaded from addon_config")
    return _secret_cache
...
async def _discover_via_supervisor(session) -> list[str]:
    ...
    for addon in (data.get("data") or {}).get("addons") or []:
        ...
        # Match every Chickadee-for-HA channel, not just the prod slug: the dev add-on's
