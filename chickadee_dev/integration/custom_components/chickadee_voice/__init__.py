...
"""Chickadee Voice — an open voice pipeline for Home Assistant.
...
def _schedule_voice_views(hass: HomeAssistant) -> None:
    """Register the /api/chickadee/voice/* gateway at HA start, with ownership guard.

    Those wire paths are a contract with shipped Chickadee apps. When BOTH halves
    ...
    async def _decide(_hass: HomeAssistant) -> None:
        ...
        if sibling_present and not ceded:
            ...
            _LOGGER.warning(
                "DROP: Chickadee Voice gateway NOT registered — the %s "
                "integration is present and has not ceded /api/chickadee/voice/* "
                "(update it to a ceding version); kiosk LAN sharing stays on "
                "the Chickadee add-on",
                DEVICE_SIBLING_DOMAIN,
            )
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    # LAN-sharing gateway for Chickadee kiosk satellites (ownership-guarded).
