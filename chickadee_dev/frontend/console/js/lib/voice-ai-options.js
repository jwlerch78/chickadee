...
const VoiceAiOptions = {
    ...
    // independent palette: cloud locality is Chickadee orange everywhere. (Until
    // 2026-07-30 the published build swapped in Chickadee's wing-blue — both
    // builds are Chickadee now, so there is nothing to branch on.)
    ...
    // The top-level Voice & AI selector: three Chickadee Intelligence presets
     ...
     *  Chickadee-cloud metered rows (John, 2026-07-12). */
    ...
    // makes it resolve even before the Chickadee repo is added). Slug hash =
    ...
    models(detection) {
        ...
        const out = [{
            ...
            group: 'Local',
            // BYO-model (Voice Master Plan WS-I): keep Chickadee's cards/tools/dialog,
    ...
    TTS: [
        ...
        // default Chickadee voice runs on Inworld (~4× cheaper per character);
     ...
     * Chickadee-for-HA box that signs in gets it back the moment it does.
     */
    hasManagedCloud() {
    ...
    SEARCH: [
        ...
        //   note: 'Enable the JSON API on your instance — add "json" to search.formats in settings.yml — so Chickadee can query it.',
