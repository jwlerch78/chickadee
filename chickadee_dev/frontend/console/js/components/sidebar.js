...
const Sidebar = {
    render(activePage) {
        ...
        // Chickadee dashboard section — built first so we can drop the whole
        ...
        // collapses entirely rather than leaving an orphaned "Chickadee" label.
     ...
     */
    /** Trial/subscription pill (delta — Chickadee builds only; open-core no-ops). */
    _renderTrialPill() {
        return window.SubscriptionStatus?.renderSidebarPill?.() ?? '';
    },

    /** Trial-start CTA nav entry (delta — Chickadee builds only; open-core no-ops). */
    _startTrialNavItem() {
        return window.DashboardTrial?.renderStartTrialNavItem?.() ?? '';
    },

    /** Post-expiry purchase nav entry (delta — Chickadee builds only; open-core no-ops). */
    _purchaseNavItem() {
