...
const App = {
    ...
    pages: {
        // Delta pages (typeof-guarded): these modules exist only in the Chickadee
    ...
    async init() {
        ...
        // e.g. the Chickadee build's checkout page) before auth, so it survives
        ...
        try {
            ...
            // when the runtime advertises the flag (the Chickadee for HA add-on
    ...
    // the Chickadee build's checkout flow uses this). We stash it before auth
     ...
     *
     *  On the published build reached over HA ingress, "no Chickadee account" is a
    ...
    _showLogin() {
        ...
        // Purchase-intent login copy (delta): the Chickadee checkout page sends
        ...
        // it (email_auth in /api/runtime; the Chickadee for HA add-on does, the Chickadee
    ...
    _showApp({ localMode = false } = {}) {
        ...
        if (localMode) {
            // No Chickadee identity to show — but HA has one, and the add-on reports
     ...
     *  SubscribeGate delta module (subscription surfaces are Chickadee-only);
