...
// contacts no Chickadee service" (PRIVACY.md's local-mode paragraph, the strongest asset of
// the r/HA positioning) and a box that quietly POSTs to Chickadee's paid gateways.
//
...
// form of "signed out, the module that names a Chickadee endpoint is never even loaded".
...
Deno.test('I4 shell: signed OUT → paidTools:false (the Chickadee-funded tools are off by rule)', () => {
...
Deno.test('I4 shell: billing stays byok in BOTH postures (the AI is never Chickadee-funded here)', () => {
...
// ── Signed out: no Chickadee service is reachable through this shell ────────────────────────

Deno.test('signed OUT: web search REFUSES rather than silently returning nothing', async () => {
  await assertRejects(() => OUT().runWebSearch('chickadee'), Error, 'without a Chickadee account');
});
...
async function capsFor(accountToken: string): Promise<{ web_search: boolean; retrieve_pictures: boolean }> {
  ...
  globalThis.fetch = ((url: string | URL | Request) => {
    ...
    // Any Chickadee call a turn attempts (balance read, image search, logging) resolves inert;
