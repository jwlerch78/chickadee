...
//   anything else                      → cloud edge fn (Chickadee credits)
//
...
const OPENAI_COMPAT = {
    ...
    // billed Chickadee credits) — the exact silent-degradation WS-I.8 forbids.
 ...
 * EXPLICIT error (degradation rule WS-I.8: never silently fall back to the metered Chickadee brain).
 *
    ...
    if (orModel) {
        ...
        if (entry && entry.key) {
            return {
                ...
                headers: { 'HTTP-Referer': 'https://github.com/jwlerch78/dashie-ha-console', 'X-Title': 'Chickadee' },
            };
        }
    }
    return null;
