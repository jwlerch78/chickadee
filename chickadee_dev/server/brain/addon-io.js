...
// Chickadee cloud edge fn runs — see src/). This file is the Chickadee add-on's I/O
// SHELL, ported from the Chickadee add-on's node-io.js.
//
...
// add-on's own Chickadee JWT (auth.getValidJwt). That single `if` is the whole gating
...
//     NOTHING here contacts a Chickadee service. PRIVACY.md's local-mode paragraph
//     ("no Chickadee service is contacted — no account, no telemetry, no version ping")
...
//
//   SIGNED IN (accountToken present) — parity with the full Chickadee add-on:
...
// Sampling temperature by call INTENT (see Chickadee 20260714_LOCAL_MODEL_BENCHMARK_RESULTS
 ...
 *  Chickadee-funded tools — so failing open here risks at most one paid tool call
 *  that debitBalance floors later. */
const FAIL_OPEN_SPEND = { spendable: true, balance: Number.POSITIVE_INFINITY, floor: 0, low: false };

/** The Chickadee connection, required LAZILY.
 *
 *  Never a top-level `require`: on the signed-out path nothing below calls this, so a
 *  box with no account never even loads the module that names a Chickadee endpoint. That
 ...
 * @param {string} [opts.accountToken]  The add-on's Chickadee JWT, or '' when signed out.
...
function createAddonIO({ endpoint, chatUrl: chatUrlOpt, model, key = '', providerLabel = '', accountToken = '', extraHeaders = {}, extraBody = {}, log = console.log }) {
    ...
    const io = {
        ...
        // never reject the turn (the AI costs Chickadee nothing). The core disables the
        // Chickadee-funded tools instead.
        billing: 'byok',
        ...
        logInteraction: (token, data) => {
            ...
            // Gemini key running gemini-*-flash would otherwise bill Chickadee credits); renders
    ...
    if (!signedIn) {
        ...
        return {
            ...
            // that kept the call away from Chickadee was the empty toolConn making the URL
