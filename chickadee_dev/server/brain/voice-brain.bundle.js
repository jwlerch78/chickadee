/* ============================================================
   ...
   Source git SHA: e28aaaf04f0ba8732bd4d5bca6c37c2a6c0af1a0
...
`;
var INQUIRY_DASHIE_HELP = `# Inquiry Context: Chickadee Product Help

The user asked about Chickadee itself \u2014 its features, settings, how to do something, or how to fix a
problem. Curated product documentation has been retrieved below. It is the ONLY trustworthy source
about Chickadee: answer from it, never from your general knowledge or the web.
...
**"How much does Chickadee cost?"** (not covered)
...
The user asked about Chickadee's personalities \u2014 either what's available, or to switch to one.
...
- Match generously on intent: "be a wizard", "talk like Dumbledore", "do the wizard voice" all
  mean \`wizard\`. Requests to go back to normal/default/"just be Chickadee" mean \`dashie\`.
...
function buildPrompt({ userRequest, inquiryType, retrievedData, context = {} }) {
  ...
  const baseValues = {
    ...
    // build names its own persona). Default 'Chickadee' — a request without
    // assistant_name renders byte-identically to the pre-variable prompt.
    ASSISTANT_NAME: context.assistantName || "Chickadee",
    DATE_TIME: dateTime,
...
async function resolvePersonality(supabase, userId, endpointId, explicitId) {
  ...
  if (!id || id === "dashie") {
    ...
    if (dflt?.family_notes) {
      return { name: "Chickadee", family_notes: dflt.family_notes };
    }
    return null;
  }
  if (UUID_RE.test(id)) {
...
async function orchestrate(deps, io, voiceCtx) {
  ...
  const context = {
    ...
    // absent → buildPrompt defaults to 'Chickadee' (byte-identical legacy prompt).
  ...
  if (p1Parsed.type === "info_request" && p1Parsed.tool === "personalities") {
    ...
    const catalogData = choices.length ? { found: true, current_personality: personality?.name ?? "Chickadee", personalities: choices } : {
      found: false,
...
module.exports.BRAIN_SOURCE_SHA = "e28aaaf04f0ba8732bd4d5bca6c37c2a6c0af1a0";
