...
export interface OrchestratorIO {
  ...
  // (add-on brain), so out-of-credits must NOT reject the turn — the AI costs Chickadee
  ...
  // endpoint — it costs Chickadee nothing), and that same signal used to be the only
...
async function orchestrate(deps: OrchestrationDeps, io: OrchestratorIO, voiceCtx: OrchestrateCtx): Promise<Turn> {
  ...
  // BYOK exception: the AI tokens are on the user's own key ($0 to Chickadee), so the
  // turn proceeds — only the Chickadee-funded tools (web/image search) are gated off
  ...
  // Two independent reasons a Chickadee-funded tool is unavailable: no credit to spend,
  ...
  // paidToolsOk gates the Chickadee-funded tools on a BYOK out-of-credits turn.
  ...
  if (p1Parsed.type === 'info_request' && p1Parsed.tool === 'home_assistant') {
    ...
    if (!entities) {
      ...
      // so the caller can fall back to its native HA path. Not a Chickadee spoken turn → no transcript.
      await logPass(io, deps, REQUEST_TYPE, req.endpoint_id, sessionId, p1Prompt, pass1);
  ...
  if (p1Parsed.type === 'info_request' && p1Parsed.tool === 'personalities') {
    ...
    const catalogData = choices.length
      ? { found: true, current_personality: personality?.name ?? 'Chickadee', personalities: choices }
      : {
        found: false,
  ...
  // Fell back to the caller's native path → not a Chickadee spoken turn → no transcript.
