...
// inside POST /adopt/:deviceId, which genuinely reads the account (it posts to Chickadee's
...
// box with no Chickadee account rendered a chat that could not see one entity (the
...
// Chickadee JWT?", a property of the box, not of the caller — two HA users on one box are
 ...
 * device_installs row in Supabase (because nobody signed into Chickadee on it).
...
router.post('/adopt/:deviceId', requireSignedIn, async (req, res) => {
    ...
    try {
        ...
        const entry = await haRegistry.getDeviceByChickadeeId(deviceId, { force: true });
        if (!entry) {
        ...
        if (!resp.ok) {
            ...
            // e.g. adopt's "already claimed by another Chickadee account". Prefer
...
async function findMediaEntity(dashieDeviceId, role) {
    const entry = await haRegistry.getDeviceByChickadeeId(dashieDeviceId);
...
/**
 * GET /api/ha/events
 * Server-Sent Events stream of HA state_changed events filtered to Chickadee-integration
...
async function _ensureStateChangedFanout() {
    ...
    sseUnsubscribe = await haRegistry.subscribeStateChanged(async (event) => {
        ...
        // Resolve to a Chickadee device + role using cached entity_registry.
        try {
            ...
            const allChickadeeIds = haWorker.getStatus().lastRun?.freshDevices?.map(d => d.device_id) || [];
            let dashieId = null;
            for (const id of allChickadeeIds) {
                const dev = await haRegistry.getDeviceByChickadeeId(id);
...
/** GET /api/ha/devices — lookup helper for debugging. Returns the per-Chickadee-id map
...
router.get('/devices', requireSignedIn, async (req, res) => {
    ...
    try {
        ...
        for (const dashieId of states) {
            const entry = await haRegistry.getDeviceByChickadeeId(dashieId);
            if (!entry) continue;
            out.push({
