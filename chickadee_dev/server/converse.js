...
//   3. a signed-in Chickadee account                              → Chickadee (metered)
...
// brain/addon-io.js. Signed out, nothing on this path contacts a Chickadee service.
...
/** Hosted route: the SAME brain, run by Chickadee under the account
...
async function converseCloud(payload, jwt) {
    ...
    try {
        ...
        return speak("I couldn't reach Chickadee just now. Please try again in a moment.");
    }
}
...
/** The add-on's own Chickadee account, or nulls when signed out. Never throws:
...
async function converse(payload) {
    ...
    if (metered && !(await capability.readHouseholdSharing())) {
        ...
        return speak(
            ...
            "Turn household sharing back on in the Chickadee panel to use it again.",
        );
    }

    if (!shell) {
        if (jwt) {
            // Route 3 is metered too — Chickadee spends the account's credits.
            ...
            if (!(await capability.readHouseholdSharing())) {
                ...
                return speak(
                    "Voice sharing is turned off for this household, so I can't use Chickadee right now. " +
                    "Turn household sharing back on in the Chickadee panel to use it again.",
                );
            }
        ...
        return speak("The Chickadee brain isn't set up yet. Sign in to Chickadee from the add-on's panel, or point me at an A.I. model server in its configuration.");
    }

    const model = shell.model;
    // accountToken is THE gate: present → the account-backed shell (tools, credit-aware
    // gating, console History); absent → the no-account shell (no Chickadee service contacted).
