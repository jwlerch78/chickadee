...
//   3. Chickadee — the account path, when signed in.
//   4. 503        — unconfigured.
//
...
const ADAPTERS = {
    elevenlabs: {
        ...
        // that has heard Chickadee's cloud voice hears the same voice here.
...
async function synthesize({ text, voice, model }) {
    ...
    console.log(`CHICKADEE-TTS route=byok provider=${provider} chars=${clean.length} bytes=${audio.length} latency=${Date.now() - t0}ms`);
